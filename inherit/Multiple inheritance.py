...
# 内置类型的原生方法使用C语言实现，不会调用子类中覆盖的方法，不过有极少数例外。
# 因此，需要定制list、dict或str类型时，子类化UserList、UserDict或UserString更简单。
...
# GradyBooch把这种类称为聚合类（aggregateclass）。

# 08.“优先使用对象组合，而不是类继承”
# 这是我能提供的最佳建议。熟悉继承之后，就太容易过度使用它了。
# 然而，优先使用组合能让设计更灵活。
