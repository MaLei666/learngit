...
class flattest():
    ...
    def hzbj(self):
        print('''
        1.上传建筑消防设施系统状态(eg:0000)
        2.上传部件运行状态(eg:0000)
        3.上传建筑消防设施操作信息(eg:02)
        4.上传传输装置运行状态(eg:02)
        5.上传传输装置操作信息(eg:02)
        6.其他
                       ''')
    ...
    def part_status(self):
        def system(sysstat):
            ...
            if playstat == ('0'):pass
            else:print('正常运行状态', end=' ')
            if powerfail == ('0'):pass
            else:print('主电故障', end=' ')
            if powerfail2 == ('0'):pass
            else:print('备电故障', end=' ')
            if delayed == ('0'):pass
            else:print('延时状态', end=' ')
            if feedback == ('0'):pass
        ...
        if self.infotp=='1':
            ...
            self.device_type='04'
            self.usedatalen = '0C00'
            self.info0='0101' #系统类型，系统地址
            self.info=partstatus
            self.info2='00'  #操作员编号
            part_useinfo(partstatus)
            self.data_deal()
    ...
    def tcpcli(self):
        ...
        tcpclisocket.connect(('111.207.68.145', 8001))
        # tcpclisocket.connect(('47.92.90.52', 8001))
